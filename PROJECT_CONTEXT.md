...
- Signer PDF access now distinguishes review from execution. Until the envelope is completed, the tokenized document route generates a non-cached review PDF with the organisation accent colour, a large `UNSIGNED DRAFT` / `NOT A VALID OR EXECUTED AGREEMENT` watermark on every page, a coloured review-only footer, the envelope reference and generation timestamp, and wording that authentic completed copies are issued and verifiable through BlendSign. Once sealing has completed, the same route serves the immutable `signedKey` PDF instead of the original upload. The signer screen labels these two states explicitly. Browser controls cannot prevent screenshots or technical extraction; the protection is the unmistakable document state plus the sealed final hash and audit trail.
