...
- Recipient mailbox evidence supplied immediately after the UAT shows three separate `Your Stor24 lease is ready to sign` messages: the original invitation plus both reminder attempts. Duplicate-envelope prevention passed, but duplicate-email suppression failed. A resend cooldown or stable reminder idempotency key must be implemented and then re-tested against the recipient mailbox and BlendSign audit trail.
...
Corrected production retest completed 31 August 2026 after Stor24 CRM commit `6feee57` deployed. The first action delivered one new invitation email at 08:49 SAST; the immediate repeat returned the server-side message `A signing reminder was sent recently. Please wait before sending another.` and no second new email appeared. The same envelope `cmtguh1t200015iu2lgmn6nrw` remained awaiting signature. This closes the duplicate-reminder suppression slice with UI and mailbox evidence.

The next bounded action is the remaining negative-test set: invalid webhook signature, unknown merge key, missing recipient role and simulated provider outage, each with visible/reconcilable failure evidence and no duplicate envelope. Retain or clean up the disposable Unit 103 record only after the final evidence pack has been preserved.

Task 2 negative-test implementation update — 31 August 2026: the live Stor24 webhook rejected a deliberately invalid completion signature with HTTP 401 and `Invalid webhook signature.` The Stor24 integration view remained at 134 processed, 0 pending, 0 failed/dead-letter and one unchanged awaiting-signature test envelope, proving no persisted state change. BlendSign's envelope route already rejects unknown merge keys and incomplete/incorrect role sets before `createEnvelopeFromTemplate`; explicit contract tests now lock those ordering invariants. Provider-outage inspection found signing-link jobs recorded `delivery_failed` and rethrew the error but lacked BullMQ retry options. Signing-link jobs now use five exponential-backoff attempts, matching other delivery jobs; contract coverage locks both failure audit and retry behaviour. Deployment and a controlled non-production provider-failure simulation remain required before the full outage slice is closed.

Controlled non-production provider-outage simulation — 31 August 2026: the signing-link handler is now dependency-injected without changing its production inputs, allowing SMTP failure behaviour to be exercised without Redis, Prisma, a real recipient or an outbound provider call. The simulation forced all five configured attempts to fail with `Simulated SMTP outage`; every attempt produced `delivery_failed`, every error propagated for BullMQ retry, and no false `sent` event was recorded. The complete contract suite passed 12/12, TypeScript passed, and the production build compiled, typechecked and generated all 43 pages. Windows standalone trace copying again reported the known OneDrive symlink `EPERM` warning after successful compilation/page generation. `next lint` remains unconfigured and prompted interactively, so it is not claimed as a passing gate. Commit/push, production deployment of this testability extraction and post-deployment login/readback remain outstanding at this checkpoint.

Task 2 final close-out — 31 August 2026: implementation, the executable outage simulation, compiler validation and the context checkpoint were committed and pushed to BlendSign `main` as `04afc58d9819d1e1cbe6239748204e1339ad8d1b`. There was no separate pull-request merge; the clean branch was promoted directly to `main`. GitHub Actions VPS deployment run `33390291116` (`#36`) checked out that exact revision and completed successfully at 14:10 SAST, rebuilding and recreating both the application and worker containers and running the existing deployment health probe. No provider configuration or production recipient was changed. Independent live readback at `2026-08-31T12:11:28.899Z` returned HTTP 200 with title `BlendSign` from `/login`; the unauthenticated `/api/v1/health` boundary correctly remained HTTP 401. Together with the earlier live invalid-HMAC and reminder-idempotency evidence plus executable pre-create unknown-key/role rejection and bounded outage simulation, the scoped BlendSign failure, security and idempotency task is complete. The known unconfigured lint gate and Windows trace-copy warning remain documented engineering hygiene items, not hidden passing gates.
